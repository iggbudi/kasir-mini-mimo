(function () {
  ...
  function openStockHistory(item) {
    ...
    const overlay = document.createElement('div'); overlay.className = 'modal-overlay';
    overlay.innerHTML = `<div class="modal stock-history-modal"><h3 class="modal-title">Riwayat Stok</h3><p class="modal-message">${KasirApp.escapeHtml(item.nama)}</p><div class="stock-history-list" data-history-list></div><div class="modal-actions mt-3"><button class="secondary" data-action="close">Tutup</button><button class="primary hidden" data-action="more">Muat Lagi</button></div></div>`;
    document.body.appendChild(overlay);
    const list = overlay.querySelector('[data-history-list]'); const more = overlay.querySelector('[data-action="more"]');
    ...
    async function loadPage() { if (loading) return; loading = true; more.disabled = true; if (!list.children.length) list.textContent = 'Memuat riwayat…'; const params = new URLSearchParams({ limit: '20', offset: String(offset) }); try { const response = await KasirApp.apiFetch(`/api/barang/${item.id}/mutasi?${params}`); const rows = response.data?.items || []; if (!offset) list.textContent = ''; render(rows); offset += rows.length; hasMore = Boolean(response.data && response.data.pagination && response.data.pagination.has_more); more.classList.toggle('hidden', !hasMore); } catch (err) { list.innerHTML = `<p class="error">${KasirApp.escapeHtml(err.message || 'Gagal memuat riwayat')}</p>`; more.textContent = 'Coba Lagi'; more.classList.remove('hidden'); } finally { loading = false; more.disabled = false; } }
