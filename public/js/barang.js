(function () {
  ...
  function openStockHistory(item) {
    ...
    const render = rows => { list.insertAdjacentHTML('beforeend', rows.map(row => { const delta = Number(row.perubahan) || 0; const date = KasirApp.escapeHtml(row.tanggal || ''); const reference = row.nomor_referensi ?? (row.referensi_id == null ? null : String(row.referensi_id)); return `<div class="stock-history-item"><strong>${KasirApp.escapeHtml(MUTATION_LABELS[row.tipe] || row.tipe || 'Mutasi')}</strong><div class="meta"><span class="${delta >= 0 ? 'stock-delta-positive' : 'stock-delta-negative'}">${delta > 0 ? '+' : ''}${delta}</span> · ${Number(row.stok_sebelum) || 0} → ${Number(row.stok_sesudah) || 0}</div>${reference != null ? `<div class="meta">Referensi: ${KasirApp.escapeHtml(String(reference))}</div>` : ''}<div class="meta">${date}${row.catatan ? ` · ${KasirApp.escapeHtml(row.catatan)}` : ''}</div></div>`; }).join('')); };
    async function loadPage() { if (loading) return; loading = true; error.classList.add('hidden'); more.disabled = true; if (!list.children.length && !list.textContent) list.textContent = 'Memuat riwayat…'; const params = new URLSearchParams({ limit: '20', offset: String(offset) }); try { const response = await KasirApp.apiFetch(`/api/barang/${item.id}/mutasi?${params}`); const rows = response.data?.items || []; if (!offset) { list.textContent = ''; if (!rows.length) list.textContent = 'Riwayat stok mulai dicatat sejak pembaruan ini; transaksi lama tidak direkonstruksi.'; } render(rows); offset += rows.length; hasMore = Boolean(response.data && response.data.pagination && response.data.pagination.has_more); more.textContent = 'Muat Lagi'; more.classList.toggle('hidden', !hasMore); } catch (err) { if (!offset) list.textContent = ''; error.textContent = err.message || 'Gagal memuat riwayat'; error.classList.remove('hidden'); more.textContent = 'Coba Lagi'; more.classList.remove('hidden'); } finally { loading = false; more.disabled = false; } }
